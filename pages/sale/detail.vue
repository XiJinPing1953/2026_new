	...
	<view class="sale-detail-page">
		<view class="sale-detail-inner">
			...
			<block v-if="!loading && record && !errorMsg">
					...
					<view class="card-body">
						...
						<view class="info-grid">
							<view class="info-cell">
								...
								<text class="info-value">{{ customerNameText }}</text>
							</view>
				...
				<!-- Truck 模式：整车信息卡片 -->
				<view v-if="isTruckMode" class="card">
					<view class="card-header">
						<text class="card-title">整车信息</text>
						<text class="card-sub">整车业务的毛重 / 皮重 / 净重记录</text>
					</view>
					<view class="card-body">
						<view class="info-row" v-if="truckInfo.gross != null">
							<text class="info-label">整车毛重</text>
							<text class="info-value">{{ formatNum(truckInfo.gross) }} kg</text>
						</view>
						<view class="info-row" v-if="truckInfo.tare != null">
							<text class="info-label">车辆皮重</text>
							<text class="info-value">{{ formatNum(truckInfo.tare) }} kg</text>
						</view>
						<view class="info-row" v-if="truckInfo.net != null">
							<text class="info-label">整车净重</text>
							<text class="info-value">{{ formatNum(truckInfo.net) }} kg</text>
						</view>
						<view class="info-row" v-if="truckInfo.sale_net != null">
							<text class="info-label">销售净重</text>
							<text class="info-value">{{ formatNum(truckInfo.sale_net) }} kg</text>
						</view>

						<view class="divider" v-if="truckInfo.out_net != null || truckInfo.back_net != null || truckInfo.fill_net != null"></view>

						<view class="info-row" v-if="truckInfo.out_net != null">
							<text class="info-label">装车净重</text>
							<text class="info-value">{{ formatNum(truckInfo.out_net) }} kg</text>
						</view>
						<view class="info-row" v-if="truckInfo.back_net != null">
							<text class="info-label">回厂净重</text>
							<text class="info-value">{{ formatNum(truckInfo.back_net) }} kg</text>
						</view>
						<view class="info-row" v-if="truckInfo.fill_net != null">
							<text class="info-label">灌装净重</text>
							<text class="info-value">{{ formatNum(truckInfo.fill_net) }} kg</text>
						</view>
					</view>
				</view>

				<!-- 瓶单模式：出瓶 / 回瓶 / 存瓶 -->
				<block v-if="!isTruckMode">
					<!-- 出瓶明细 -->
					<view class="card">
						<view class="card-header">
						...
						<view class="formula-section">
							<view class="formula-block">
								...
								<view class="formula-line">
									...
									<text class="formula-main">{{ outNetTotalText }} kg</text>
								</view>

								<view class="formula-line" v-if="backList.length">
									<text class="formula-label">回瓶净重</text>
									<text class="formula-main">{{ backNetTotalText }} kg</text>
								</view>

								<view class="formula-line">
									<text class="formula-label">净重合计</text>
									<text class="formula-main">{{ netTotalText }} kg</text>
								</view>
							</view>
							...
							<view class="formula-block">
								...
								<view class="formula-line">
									...
									<text class="formula-main">{{ outAmountText }} 元</text>
								</view>

								<view class="formula-line" v-if="isKgPrice && backNetTotalRaw !== 0">
									<text class="formula-label">回瓶结算</text>
									<text class="formula-main">{{ backAmountText }} 元</text>
								</view>

								<view class="formula-line">
									<text class="formula-label">理论应收</text>
									<text class="formula-main">{{ shouldReceiveText }} 元</text>
								</view>
							</view>
						</view>
	...
	export default {
		...
		computed: {
			customerNameText() {
				...
				return this.record.customer_name || this.record.customerName || '—'
			},
			...
			isTruckMode() {
				if (!this.record) return false
				const mode = (this.record.biz_mode || this.record.bizMode || '').toLowerCase()
				return mode === 'truck'
			},
			truckInfo() {
				if (!this.record) return {}
				...
				const pick = (...vals) => {
					for (const v of vals) {
						if (this.hasWeightValue(v)) return Number(v)
					}
					return null
				}
				return {
					gross: pick(info.gross, this.record.truck_gross),
					tare: pick(info.tare, this.record.truck_tare),
					net: pick(info.net, this.record.truck_net),
					sale_net: pick(info.net_sale_kg, this.record.truck_net_sale_kg),
					out_net: pick(info.out_net, this.record.truck_out_net),
					back_net: pick(info.back_net, this.record.truck_back_net),
					fill_net: pick(info.fill_net, this.record.truck_fill_net)
				}
			},
			truckNetSaleKg() {
				...
				const info = this.record.truck_info || {}
				const val =
					info.net_sale_kg != null
						? info.net_sale_kg
						: this.record.truck_net_sale_kg
				return this.toNum(val, 0)
			},
			truckNetSaleKgText() {
				return this.truckNetSaleKg.toFixed(2)
			},
			...
			outAmountFormula() {
				if (this.isTruckMode) {
					return `${this.truckNetSaleKgText} × ${this.unitPriceNumberText} = ${this.outAmountText} 元`
				}
				if (this.isKgPrice) {
			...
			outList() {
				if (!this.record) return []
				if (Array.isArray(this.record.out_items) && this.record.out_items.length) {
					return this.normalizeBottleRows(this.record.out_items)
				}
			...
			backList() {
				if (!this.record) return []
				if (Array.isArray(this.record.back_items) && this.record.back_items.length) {
					return this.normalizeBottleRows(this.record.back_items)
				}
			...
			outAmountRaw() {
				if (!this.record) return 0
				if (this.isTruckMode) {
					return this.truckNetSaleKg * this.unitPriceNumber
				}
			...
			shouldReceiveRaw() {
				if (!this.record) return 0
				if (this.isTruckMode) return this.outAmountRaw
		...
		methods: {
			...
			normalizeBottleRows(list) {
				return (list || [])
					.map(item => {
						if (item && typeof item === 'object') {
							const bottleNo = item.bottle_no != null ? item.bottle_no : item.number != null ? item.number : ''
							return {
								bottle_no: bottleNo,
								gross: item.gross ?? item.gross_weight ?? item.grossWeight ?? item.gross_weight_out ?? item.gross_weight_back,
								tare: item.tare ?? item.tare_weight ?? item.tareWeight ?? item.tare_weight_out ?? item.tare_weight_back,
								net: item.net ?? item.net_weight ?? item.netWeight ?? item.net_weight_out ?? item.net_weight_back
							}
						}
						const text = item == null ? '' : String(item)
						return { bottle_no: text, gross: null, tare: null, net: null }
					})
					.filter(row => row.bottle_no || this.hasWeightValue(row.gross) || this.hasWeightValue(row.tare) || this.hasWeightValue(row.net))
			},
