            ...
            >
              ...
              <view class="sale-col col-customer">
                <text class="cell-main">
                  {{ item.customer_name || item.customerName || '未知客户' }}
                </text>
                <text class="cell-sub">
...
export default {
  ...
  methods: {
    ...
    onSelectCustomer(item) {
      ...
      this.filters.customerId = cid
      this.filters.customerName = item.name
      this.customerSuggests = []
      console.log('[sale/list] selected customerId:', cid)
    },
    ...
    async loadCustomers() {
      try {
        ...
        if (result.code === 0) {
          ...
          this.customers = list
          console.log('[sale/list] customers loaded:', list.length, list[0])
    ...
    onCustomerChange(e) {
      ...
      if (c) {
        this.filters.customerId = String(c._id || '')
