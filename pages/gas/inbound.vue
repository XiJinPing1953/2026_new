			...
			<view class="card card-filter">
				<view class="filter-row">
					...
					<view class="filter-item keyword">
						...
						<view class="input-wrapper">
							<input class="input" v-model="filters.keyword" placeholder="按车牌 / 罐车号 / 液厂搜索"
								confirm-type="search" @confirm="loadList" />
						</view>
					</view>
					...
					<view class="edit-body">
						...
						<view class="form-row">
							<view class="form-item third">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.load_weight" type="digit" placeholder="吨"
										@blur="autoCalcNet" />
								</view>
							</view>
							...
							<view class="form-item third">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.gross_weight" type="digit" placeholder="吨"
										@blur="autoCalcNet" />
								</view>
							</view>
							...
							<view class="form-item third">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.tare_weight" type="digit" placeholder="吨"
										@blur="autoCalcNet" />
								</view>
							</view>
						</view>
						...
						<view class="form-row">
							<view class="form-item half">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.net_weight" type="digit" placeholder="毛重 - 皮重" />
								</view>
							</view>
							...
							<view class="form-item half">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.loss_amount" type="digit" placeholder="吨，可为 0" />
								</view>
							</view>
						</view>
						...
						<view class="form-row">
							<view class="form-item half">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.unit_price" type="digit" placeholder="元/吨"
										@blur="autoCalcAmount" />
								</view>
							</view>
							...
							<view class="form-item half">
								...
								<view class="input-wrapper">
									<input class="input" v-model="form.amount" type="digit" placeholder="可自动=净重×单价" />
								</view>
							</view>
						</view>
						...
						<view class="form-item">
							...
							<textarea class="textarea" v-model="form.remark" placeholder="可记录票号、上下游结算说明等" />
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			autoCalcLoss() {
				const load = Number(this.form.load_weight) || 0
				const net = Number(this.form.net_weight) || 0

				// 两个都有值时才算（避免一边没填就把亏损刷成 0）
				if (load && net) {
					const loss = load - net
					this.form.loss_amount = loss >= 0 ? loss.toFixed(2) : ''
				}
			},
			// 统一两位小数
			format2(v) {
			...
			normalizeTonNumber(value, label, {
				required = false
			} = {}) {
			...
			async submitEdit() {
				...
				const gross = this.normalizeTonNumber(this.form.gross_weight, '毛重', {
					required: true
				})
				if (gross === false) return
				const tare = this.normalizeTonNumber(this.form.tare_weight, '皮重', {
					required: true
				})
				...
				if (net != null && gross != null && tare != null && net < 0) {
					return uni.showToast({
						title: '净重不能为负，请检查毛重/皮重',
						icon: 'none'
					})
				}

				let lossAmount = this.normalizeTonNumber(this.form.loss_amount, '亏损')
				if (lossAmount === null && loadWeight != null && net != null) {
					lossAmount = Number((loadWeight - net).toFixed(3))
				}
				if (lossAmount === false) return

				const unitPrice = this.normalizePrice(this.form.unit_price, '单价', {
					required: true
				})
