        ...
        <view class="form-item col-third">
          ...
          <view v-if="customerSuggests.length" class="sbic-suggest">
            <view
              ...
              @mousedown.stop.prevent="onSelectCustomer(item)"
            >
...
export default {
  ...
  watch: {
    value: {
      ...
      handler(v) {
        ...
        this.customerKeyword = h.customer_name || h.customerName || ''
  ...
  methods: {
    ...
    onCustomerInput(e) {
      ...
      // 仅输入时：先写回 customer_name，并清空 customer_id（避免脏 id）
    ...
    // ✅ 选中客户：自动代入“默认单价&单位”，但分别尊重手动锁
    // - 用户手动改过单价 => 不再覆盖单价；但仍允许单位随客户变化
    // - 用户手动改过单位 => 不再覆盖单位；但仍允许单价随客户变化
    onSelectCustomer(item) {
      ...
      const defaultPrice = item.default_unit_price
      const defaultUnit = item.default_price_unit || 'kg' // kg / bottle / m3
      ...
      const priceManual = this.header.unit_price_manual === true
      const unitManual = this.header.price_unit_manual === true

      if (!priceManual) {
        patch.unit_price =
          (defaultPrice != null && defaultPrice !== '') ? String(defaultPrice) : ''
      }

      if (!unitManual) {
        patch.price_unit = defaultUnit
        const map = { kg: 0, bottle: 1, cbm: 2, m3: 2 }
        this.priceUnitIndex = map[defaultUnit] != null ? map[defaultUnit] : 0
      }

      // 可选：保留总开关（不改变既有状态）
      patch.price_manual =
        this.header.price_manual === true ||
        this.header.unit_price_manual === true ||
        this.header.price_unit_manual === true
