...
exports.main = async (event, context) => {
  ...
  if (action === 'createV2') {
    ...
    const truck_out_gross = toNumber(truckOutGross)
    const truck_back_gross = toNumber(truckBackGross)
    const truck_sale_net = toNumber(truckSaleNet || truckNet || 0)
    const truck_loss = toNumber((truck_out_gross - truck_back_gross) - truck_sale_net, 0)

    const truckBottleNoRaw = (payload.truck_no || '').trim() || (car_no || '').trim()
    const truck_bottle_no = truckBottleNoRaw ? truckBottleNoRaw.toUpperCase() : 'TRUCK-NO'

    if (biz_mode === 'truck') {
      // 确保 out_items 至少有一条 TRUCK 号记录，便于瓶子状态同步
      const exist = out_items.find(it => (it.bottle_no || '').trim() === truck_bottle_no)
      if (exist) {
        if (!exist.net) exist.net = truck_sale_net
        if (!exist.gross && truck_out_gross) exist.gross = truck_out_gross
      } else {
        out_items.unshift({
          bottle_no: truck_bottle_no,
          gross: truck_out_gross || truck_gross || null,
          tare: null,
          net: truck_sale_net || truck_net || 0,
          bottle_id: null
        })
      }
    }
    ...
    out_items.forEach(it => {
      ...
      if (!no) return
      if (biz_mode === 'truck') return // 整车业务不参与存瓶
    ...
    const doc = {
      ...
      truck_back_gross,
      truck_sale_net,
      truck_net_sale_kg: truck_sale_net,
      truck_loss,
    ...
    if (biz_mode === 'truck' && truck_bottle_no) {
      // 回厂毛重填写视为已回厂
      if (truck_back_gross) {
        bottleTasks.push(
          updateBottleStatusByNumber(truck_bottle_no, {
            status: 'in_station',
            last_customer_id: customer_id || '',
            last_customer_name: customer_name || '',
            last_back_date: date
  ...
  if (action === 'listByBottle') {
    ...
    (res.data || []).forEach(doc => {
      const safeNum = v => (v === 0 ? 0 : v ? Number(v) : null)
      ...
      const base = {
        ...
        should_receive: doc.should_receive != null ? Number(doc.should_receive) : (doc.amount != null ? Number(doc.amount) : null),
        out_amount: doc.out_amount != null ? Number(doc.out_amount) : null,
        back_amount: doc.back_amount != null ? Number(doc.back_amount) : null,
        truck_out_gross: safeNum(doc.truck_out_gross ?? doc.truck_gross),
        truck_back_gross: safeNum(doc.truck_back_gross),
        truck_sale_net: safeNum(doc.truck_sale_net ?? doc.truck_net_sale_kg ?? doc.truck_net),
        truck_loss: safeNum(doc.truck_loss)
      }
      ...
      const bizMode = doc.biz_mode || 'bottle'
      ...
      } else if (bizMode === 'truck' && base.truck_sale_net != null) {
        rows.push({
          ...base,
          role: 'out',
          role_text: '出厂',
          net_weight: base.truck_sale_net,
          deposited: false
        })
      }
      ...
      } else if (bizMode === 'truck' && base.truck_back_gross != null) {
        rows.push({
          ...base,
          role: 'back',
          role_text: '回厂',
          net_weight: base.truck_back_gross
        })
      }
